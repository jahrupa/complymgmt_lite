...
const Location = () => {
    ...
    const [current, setCurrent] = useState({ id: null, company_name: '', group_holding: '', role: '', location: "", status: '', desc: '', access_modules: [] });
    ...
    const handleSubmit = (e) => {
        ...
        if (isEditing) {
            const updatedData = data.map((item) =>
                item.id === current.id ? { ...item, company_name: current.company_name, group_holding: current.group_holding, role: current.role, location: current.location, status: current.status, desc: current.desc, access_modules: current.access_modules } : item
            );
            setData(updatedData);
        } else {
            const newData = { id: Date.now(), company_name: current.company_name, group_holding: current.group_holding, role: current.role, location: current.location, status: current.status, desc: current.desc, access_modules: current.access_modules };
            setData((prev) => [...prev, newData]);
        }
        setCurrent({ id: null, company_name: '', group_holding: '', role: '', location: '', status: '', desc: '', access_modules: [] });
        setIsEditing(false);
    ...
    const crudForm = () => {
        return (
            <div>
                ...
                <div className=''>
                    <SingleSelectTextField name="company_name" label="Company Name" value={current.company_name} onChange={(e) => setCurrent((prev) => ({ ...prev, company_name: e.target.value }))} names={groupHolding} />
                    
                </div>
                <div>
                    <SingleSelectTextField name="group_holding" label="Grop Holding" value={current.group_holding} onChange={(e) => setCurrent((prev) => ({ ...prev, group_holding: e.target.value }))} names={groupHolding} />
            ...
            <div className='table_div p-3'>
                <div className='d-lg-flex d-md-flex  justify-content-between'>
                    <div className='d-flex h-100'>
                        ...
                        <MultipleSelectFields placeholder='Filter By Location' roleName={groupHolding} />
                    </div>
