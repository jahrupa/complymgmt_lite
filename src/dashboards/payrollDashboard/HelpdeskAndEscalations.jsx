...
import { fetchHelpDeskPendingForSelectedIssueSubtypes, fetchHelpdeskStatusBasedOnIssueSubType, fetchHelpdeskTicketsRaisedByCompany, fetchTicketsDistributionAssignedToCount, fetchTotalCountOfCommunicationTypes, fetchTotalDelayFlagsByClientAndGovt, fetchTotalDelayFlagsByGovt } from '../../api/service';
...
const HelpdeskAndEscalations = ({ selectedCompany }) => {
    ...
    const closeVsOpenIssueFormat = {
        series: [{
            name: "Open Cases",
            data: closedVsOpenCases?.top_issue_sub_type_status?.map((item) => item.count_Open)
        },
        {
            name: "Closed Cases",
            data: closedVsOpenCases?.top_issue_sub_type_status?.map((item) => item.count_Closed)
        },],
        options: {
            chart: {
                type: "bar",
            ...
            xaxis: {
                categories: closedVsOpenCases?.top_issue_sub_type_status?.map((item) => item.issue_sub_type || []) || [],
                labels: {
    ...
    const ProportionOfCasesFormat = {
        series: [
            {
                name: "Count",
                data: ProportionOfCases?.top_count?.map((item) => item.count_issue_types ?? 0) || [],
            },
        ...
        options: {
            ...
            xaxis: {
                categories: ProportionOfCases?.top_count?.map((item) => item.pending_from ?? 0) || [],
                labels: {
                    formatter: function (val) {
                        return val + "K"
                    }
    ...
    const [clientVsGovernmentDelayFlags, setClientVsGovernmentDelayFlags] = React.useState({

    });
    const clientVsGovernmentDelayFlagsFormat = {
        series: clientVsGovernmentDelayFlags?.client_delay?.map((item) => item.count) || [],
        options: {
            chart: {
                width: 380,
                type: "donut",
            ...
            labels: clientVsGovernmentDelayFlags?.client_delay?.map((item) => item.delay_type) || [],
            legend: {
    ...
    const governmentDelayFlagsFormat = {
        ...
        options: {
            ...
            labels: governmentDelayFlags?.govt_delay?.map((item) => item.delay_type) || [],
            legend: {
                position: "top", // 👈 moves Yes/No below the chart
                horizontalAlign: "center",
                fontSize: "14px",
    ...
    useEffect(() => {
        const fetchData = async () => {
            const [communicationTypeRes, helpdeskTicketsRes, helpdeskStatusRes, ticketsDistributionRes, helpdeskPendingRes, totalDelayFlags, totalDelayFlagsByGovt] = await Promise.allSettled([
                ...
                fetchTotalDelayFlagsByClientAndGovt(selectedCompany),
            ...
            if (helpdeskPendingRes.status === 'fulfilled') {
                ...
                console.warn("fetchAll helpdesk pending failed:", helpdeskPendingRes.reason);
                setProportionofcases(helpdeskPendingRes.reason?.status || []);
            }
            if (totalDelayFlags.status === 'fulfilled') {
                setClientVsGovernmentDelayFlags(totalDelayFlags.value);
            } else {
                console.warn("fetchAll delay flags failed:", totalDelayFlags.reason);
                setClientVsGovernmentDelayFlags(totalDelayFlags.reason?.status || []);
            }
            if (totalDelayFlagsByGovt.status === 'fulfilled') {
                setGovernmentDelayFlags(totalDelayFlagsByGovt.value);
            } else {
                console.warn("fetchAll delay flags failed:", totalDelayFlagsByGovt.reason);
        ...
        <div>
            <div className='charts-grid mb-4'>
                <div className={`chart-card ${selectedCharts.includes("ps-1") ? "selected-card" : ""}`}
                        ...
                        <div className='dashboard-icon ms-2'
                            onClick={() =>
                                handleOpenDrawer(
                                    ...
                                    ProportionOfCases?.rest_count?.map((item) => item.pending_from),
                                )
                            }
                        >
                            <ArrowUpRight />
                    ...
                    <div className="mb-3 fw-600">Top 5 Proportion of cases pending from department vs. pending from client for selected Issue Sub-Types in PF_ESIC_Helpdesk</div>
                    <Chart
                        options={ProportionOfCasesFormat.options} series={ProportionOfCasesFormat.series} type="bar" height={380}
                    />
                </div>
                <div className={`chart-card ${selectedCharts.includes("ps-1") ? "selected-card" : ""}`}
                    onClick={() => toggleChartSelection("ps-1")}
                        ...
                        <div className='dashboard-icon ms-2'
                            onClick={() =>
                                handleOpenDrawer(
                                    ...
                                    closedVsOpenCases?.rest_issue_sub_type_status?.map((item) => item.issue_sub_type),
                                )
                            }
                        >
                            <ArrowUpRight />
                    ...
                    <div className="mb-3 fw-600">Top 5 Comparison of closed vs. open cases for top 5 Issue Sub-Types in PF_ESIC_Helpdesk</div>
                    <Chart
            ...
            <div className='charts-grid mb-4'>
                <div className="chart-card">
                    <div className="mb-3 fw-600">Analysis of client delay flags for open tickets in PF_ESIC_Helpdesk</div>
                    <Chart
                        options={clientVsGovernmentDelayFlagsFormat.options} series={clientVsGovernmentDelayFlagsFormat.series} type="donut" height={380}
                    />
                </div>
                <div className="chart-card">
                        ...
                        <div className='dashboard-icon ms-2'
                            onClick={() =>
                                handleOpenDrawer(
                                    ...
                                    ticketDistribution?.rest_assigned_to_counts?.map((item) => item.assigned_to),
                                )
                            }
                        >
                            <ArrowUpRight />
            ...
            <div className='charts-grid mb-4'>
                <div className="chart-card">
                    ...
                    <Chart
                        options={governmentDelayFlagsFormat.options} series={governmentDelayFlagsFormat.series} type="donut" height={380}
                    />
                </div>
                        ...
                        <div className='dashboard-icon ms-2'
                            onClick={() =>
                                handleOpenDrawer(
                                    ...
                                    helpdeskTicketsRaisedByCompany?.rest_company_tickets_count?.map((item) => item.company_name),
                                )
                            }
                        >
                            <ArrowUpRight />
